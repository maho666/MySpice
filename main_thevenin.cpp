...
void thevenin(vector <vector<component>> &circuit,string &stage,int &stage_num,vector<string> &voltage_unit,vector<string> &current_unit)
{
    double value_no_unit,temp_value,temp_value2,temp_voltage,temp_current;
    vector<double> one_value,new_value;
    vector<double> voltage,current;
    ...
    double R1 = new_value[0]-new_value[1];
    double R_th = (R1*new_value[1])/(R1+new_value[1]);
    string R_th_unit=unit_change(R_th,0);//thevenin R
    circuit[1][0].value = R_th_unit;
    temp_value = stod(circuit[0][0].value);////get voltage
    new_value[0] = (temp_value/(new_value[0]))*new_value[1];

    string value_unit = unit_change(new_value[0],0);//get unit name head(no change) tail value
    circuit[0][0].value = value_unit;// thevenin voltage value
    string  new_resister;
    circuit[1][0].resister = "Rth";
    circuit[1][0].resister.append(new_resister);

    outClientFile << "***Thevenin equivalence circuit***"<<endl;
    outClientFile <<circuit[1][0].resister<<" = "<<circuit[1][0].value<<endl;
    outClientFile <<"Vth"<<" = "<<circuit[0][0].value<<"V"<<endl;
    outClientFile.close();
    cout<<"The bonus file successfully exported!"<<endl;
}
...
int main()
{
    ...
    vector<vector<component>> circuit;////2d whole circuit
    vector<string>voltage_unit,current_unit;////vector & current with unit
    ...
    switch(func)
    {
        ...
        case 7:
           ...
           thevenin(circuit,stage,stage_num,voltage_unit,current_unit);
        break;
